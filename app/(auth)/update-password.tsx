...
export default function UpdatePasswordScreen() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      ...
      else setError('Enlace inválido o caducado. Solicita un nuevo restablecimiento.')
    })
  }, [])
  ...
  async function handleUpdate() {
    ...
    try {
      ...
      setSuccess(true)
      if (!redirectRef.current) { redirectRef.current = true; setTimeout(() => router.replace('/(tabs)'), 1500) }
    } catch (err: any) {
  ...
  if (!ready) {
    return (
      <View style={styles.container}><View style={styles.center}><Text style={[styles.subtitle, { color: error ? colors.danger : colors.textTertiary }]}>{error || 'Verificando enlace...'}</Text>
        {error ? <TouchableOpacity style={styles.button} onPress={() => router.replace('/(auth)/forgot-password')}><Text style={styles.buttonText}>Solicitar nuevo enlace</Text></TouchableOpacity> : null}
      </View></View>
    )
  }
  ...
  if (success) {
    return (
      <View style={styles.container}><View style={styles.center}>
        <Text style={[styles.title, { fontSize: 24 }]}>Contraseña actualizada</Text>
    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={styles.content}>
        <View style={styles.card}>
          ...
          <TextInput style={[styles.input, error && styles.inputError]} placeholder="Nueva contraseña"
            placeholderTextColor={colors.inputPlaceholder} value={password}
            onChangeText={(v) => { setPassword(v); setError('') }} secureTextEntry />
          <TextInput style={[styles.input, error && styles.inputError]} placeholder="Confirmar contraseña"
            placeholderTextColor={colors.inputPlaceholder} value={confirmPassword}
            onChangeText={(v) => { setConfirmPassword(v); setError('') }} secureTextEntry />
